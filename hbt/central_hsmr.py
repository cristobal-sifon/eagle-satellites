...
def hsmr_snap(args, sim, reader, isnap, mstarbins,
              logMmin=9, logMstar_min=9, logM200Mean_min=9):
...
def plot_hsmr_ratio(args, sim, isnap_array, xbins, chsmr, shsmr, thin=5):
    ...
    for j, (i, dashes) in enumerate(
        ...
        axes[0].plot(xo, shsmr[0,i], color, lw=4, dashes=dashes,
...
def write_hsmr(args, sim, isnap_array, chsmr, shsmr, mstarbins):
    ...
    for x, name in zip((chsmr,shsmr), ('chsmr','shsmr')):
        for xstat, stat in zip(x, ('mean','std')):
            hsmr = np.vstack(
                [isnap_array, sim.t_lookback[isnap_array].value,
                sim.redshifts[isnap_array], xstat.T]).T
