...
def main():
    ...
    distribution(
        args, subs, ('Mbound','Mstar'), ['time'])

    return


def distribution(args, subs, cols, bin_cols, bin_when='first_infall',
                 avg_when='first_infall'):
    ...
    X = binstat_dd(
        [subs[bc] for bc in bin_cols], [subs[c] for c in cols], 'mean', bins).statistic
    ic(X.shape)
    return X
